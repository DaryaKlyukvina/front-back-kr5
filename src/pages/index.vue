<template>
    <div class="generator-page">
        ...
        <!-- Комбинированное модальное окно импорта/экспорта -->
        <Teleport to="body">
            <ExportImportModal
                v-if="showExportModal || showImportModal"
                :colors="currentColors"
                :palette-name="saveName || 'My Palette'"
                @close="showExportModal = false; showImportModal = false"
                @import="handleImport" />
        </Teleport>
...
const handleShare = async () => {
    if (currentColors.value.length === 0) {
        copyToClipboard("Сначала создайте палитру");
        return;
    }

    const url = generateShareURL(currentColors.value);
    shareUrl.value = url;
